import { AndroidIcon } from "../../../svgs/androidIcon";
import { CursorLight } from "../../../svgs/cursorLight";
import { GithubLight } from "../../../svgs/githubLight";
import { NextjsLogoLight } from "../../../svgs/nextjsLogoLight";
import { ReactLight } from "../../../svgs/reactLight";
import { Vercel } from "../../../svgs/vercel";

const TailwindCSS = (props) => (
  <svg {...props} fill="none" viewBox="0 0 54 33">
    <g clipPath="url(#tailwindcss__a)">
      <path
        fill="#38bdf8"
        fillRule="evenodd"
        d="M27 0c-7.2 0-11.7 3.6-13.5 10.8 2.7-3.6 5.85-4.95 9.45-4.05 2.054.513 3.522 2.004 5.147 3.653C30.744 13.09 33.808 16.2 40.5 16.2c7.2 0 11.7-3.6 13.5-10.8-2.7 3.6-5.85 4.95-9.45 4.05-2.054-.513-3.522-2.004-5.147-3.653C36.756 3.11 33.692 0 27 0zM13.5 16.2C6.3 16.2 1.8 19.8 0 27c2.7-3.6 5.85-4.95 9.45-4.05 2.054.514 3.522 2.004 5.147 3.653C17.244 29.29 20.308 32.4 27 32.4c7.2 0 11.7-3.6 13.5-10.8-2.7 3.6-5.85 4.95-9.45 4.05-2.054-.513-3.522-2.004-5.147-3.653C23.256 19.31 20.192 16.2 13.5 16.2z"
        clipRule="evenodd"
      />
    </g>
  </svg>
);

const GoogleCloud = (props) => (
  <svg {...props} preserveAspectRatio="xMidYMid" viewBox="0 -25 256 256">
    <path
      fill="#EA4335"
      d="m170.252 56.819 22.253-22.253 1.483-9.37C153.437-11.677 88.976-7.496 52.42 33.92 42.267 45.423 34.734 59.764 30.717 74.573l7.97-1.123 44.505-7.34 3.436-3.513c19.797-21.742 53.27-24.667 76.128-6.168l7.496.39Z"
    />
    <path
      fill="#4285F4"
      d="M224.205 73.918a100.249 100.249 0 0 0-30.217-48.722l-31.232 31.232a55.515 55.515 0 0 1 20.379 44.037v5.544c15.35 0 27.797 12.445 27.797 27.796 0 15.352-12.446 27.485-27.797 27.485h-55.671l-5.466 5.934v33.34l5.466 5.231h55.67c39.93.311 72.553-31.494 72.864-71.424a72.303 72.303 0 0 0-31.793-60.453"
    />
    <path
      fill="#34A853"
      d="M71.87 205.796h55.593V161.29H71.87a27.275 27.275 0 0 1-11.399-2.498l-7.887 2.42-22.409 22.253-1.952 7.574c12.567 9.489 27.9 14.825 43.647 14.757"
    />
    <path
      fill="#FBBC05"
      d="M71.87 61.425C31.94 61.664-.237 94.228.001 134.159a72.301 72.301 0 0 0 28.222 56.88l32.248-32.246c-13.99-6.322-20.208-22.786-13.887-36.776 6.32-13.99 22.786-20.208 36.775-13.888a27.796 27.796 0 0 1 13.887 13.888l32.248-32.248A72.224 72.224 0 0 0 71.87 61.425"
    />
  </svg>
);

const Laravel = (props) => (
  <svg {...props} preserveAspectRatio="xMidYMid" viewBox="0 0 256 264">
    <path
      d="m255.9 59.6.1 1.1v56.6c0 1.4-.8 2.8-2 3.5l-47.6 27.4v54.2c0 1.4-.7 2.8-2 3.5l-99.1 57-.7.4-.3.1c-.7.2-1.4.2-2.1 0l-.4-.1-.6-.3L2 206c-1.3-.8-2.1-2.2-2.1-3.6V32.7l.1-1.1.2-.4.3-.6.2-.4.4-.5.4-.3c.2 0 .3-.2.5-.3L51.6.6c1.3-.8 2.9-.8 4.1 0L105.3 29c.2 0 .3.2.4.3l.5.3c0 .2.2.4.3.5l.3.4.3.6.1.4.2 1v106l41.2-23.7V60.7c0-.4 0-.7.2-1l.1-.4.3-.7.3-.3.3-.5.5-.3.4-.4 49.6-28.5c1.2-.7 2.8-.7 4 0L254 57l.5.4.4.3.4.5.2.3c.2.2.2.5.3.7l.2.3Zm-8.2 55.3v-47l-17.3 10-24 13.7v47l41.3-23.7Zm-49.5 85v-47l-23.6 13.5-67.2 38.4v47.5l90.8-52.3ZM8.2 39.9V200l90.9 52.3v-47.5l-47.5-26.9-.4-.4c-.2 0-.3-.1-.4-.3l-.4-.4-.3-.4-.2-.5-.2-.5v-.6l-.2-.5V63.6L25.6 49.8l-17.3-10Zm45.5-31L12.4 32.8l41.3 23.7 41.2-23.7L53.7 8.9ZM75 157.3l24-13.8V39.8l-17.3 10-24 13.8v103.6l17.3-10ZM202.3 36.9 161 60.7l41.3 23.8 41.3-23.8-41.3-23.8Zm-4.1 54.7-24-13.8-17.3-10v47l24 13.9 17.3 10v-47Zm-95 106 60.6-34.5 30.2-17.3-41.2-23.8-47.5 27.4L62 174.3l41.2 23.3Z"
      fill="#FF2D20"
    />
  </svg>
);

// SVG Icon component that renders SVG from string
const SVGIcon = ({ svgContent, className }) => {
  if (!svgContent) return null;

  return (
    <div
      className={className}
      dangerouslySetInnerHTML={{ __html: svgContent }}
    />
  );
};

const TechnologyField = ({ technologies, pageSettings }) => {
  const iconComponents = {
    AndroidIcon,
    CursorLight,
    GithubLight,
    NextjsLogoLight,
    TailwindCSS,
    ReactLight,
    Vercel,
    Laravel,
    GoogleCloud,
  };

  const skills = [
    {
      svgContent: pageSettings?.android_icon_svg,
      fallback: 'AndroidIcon'
    },
    {
      svgContent: pageSettings?.cursor_icon_svg,
      fallback: 'CursorLight'
    },
    {
      svgContent: pageSettings?.github_icon_svg,
      fallback: 'GithubLight'
    },
    {
      svgContent: pageSettings?.nextjs_icon_svg,
      fallback: 'NextjsLogoLight'
    },
    {
      svgContent: pageSettings?.tailwind_icon_svg,
      fallback: 'TailwindCSS'
    },
    {
      svgContent: pageSettings?.react_icon_svg,
      fallback: 'ReactLight'
    },
    {
      svgContent: pageSettings?.vercel_icon_svg,
      fallback: 'Vercel'
    },
    {
      svgContent: pageSettings?.laravel_icon_svg,
      fallback: 'Laravel'
    },
    {
      svgContent: pageSettings?.google_cloud_icon_svg,
      fallback: 'GoogleCloud'
    },
  ];

  return (
    <div className="bg-white py-18">
      <div className="w-11/12 lg:w-9/12 mx-auto text-slate-900">
        <div className="grid grid-cols-1 lg:grid-cols-2 place-items-center gap-10">
          <div>
            <h1 className="text-5xl font-semibold mb-8">
              {pageSettings?.contribution_title || 'Contribution to the field of technology'}
            </h1>

            <p>{pageSettings?.contribution_description?.split('\n').map((line, index) => (
              <span key={index}>{line}<br /></span>
            )) || 'Shahriar Khan has made significant contributions to the field of technology through innovative solutions and leadership in digital transformation.'}</p>
          </div>

          <div>
            <div className="relative z-10">
              <img src={pageSettings?.contribution_image || "/assets/technology/contribute_field.png"} alt="" />
              <div className="absolute hidden lg:block -bottom-4 -left-10 -z-10">
                <img src="/assets/technology/frame.svg" alt="" />
              </div>
            </div>
          </div>
        </div>
      </div>

      <div className="w-11/12 mx-auto lg:w-9/12 py-18">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-20">
          <div className="lg:col-span-1">
            <h1 className="text-4xl lg:text-6xl font-semibold text-slate-950 mb-6">
              {pageSettings?.tools_title || 'Tools and software skills'}
            </h1>

            <p className="text-slate-900">
              {pageSettings?.tools_description || 'Shahriar Khan is proficient in various technology tools and platforms essential for modern software development and system administration. His expertise includes containerization technologies like Docker and Kubernetes, as well as modern frameworks such as React, Next.js, Node.js, and Python. He leverages tools like GitHub for version control, Vercel for deployment, and Vite for fast development workflows.'}
            </p>
          </div>

          <div className="lg:col-span-2">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              {skills.map((skill, index) => {
                const IconComponent = iconComponents[skill.fallback];
                return (
                  <div key={index}>
                    <div className="bg-background border py-8 px-4 rounded-xl h-64 w-full hover:bg-primary-foreground">
                      <div className="flex items-center justify-center h-full">
                        {skill.svgContent ? (
                          <img src={skill.svgContent} alt="" className="h-24 w-24" />
                        ) : IconComponent ? (
                          <IconComponent className="h-24 w-24" />
                        ) : (
                          <div className="h-24 w-24 bg-gray-200 rounded flex items-center justify-center text-gray-500 text-sm">
                            Icon
                          </div>
                        )}
                      </div>
                    </div>
                  </div>
                );
              })}
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default TechnologyField;
