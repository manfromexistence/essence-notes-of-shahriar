...
function SortableLogoItem({
    ...
    return (
        <div
            ref={setNodeRef}
            style={style}
            className={`border rounded-lg p-4 hover:shadow-md transition-shadow bg-white dark:bg-gray-900 ${isDragging ? 'shadow-lg' : ''}`}
        >
...
export default function IndexPageManagement({ indexPage }: Props) {
    ...
    const [logos, setLogos] = useState<Logo[]>(indexPage?.logos || []);
    ...
    // Handle drag end for logo reordering
    ...
    // Save logo order to server
    const saveLogoOrder = (orderedLogos: Logo[]) => {
        const logoData = orderedLogos.map((logo) => ({
            id: logo.id,
            display_order: logo.display_order,
        }));

        fetch("/admin/index-page/logos/reorder", {
            method: 'POST',
            body: JSON.stringify({ logos: logoData }),
            headers: {
            ...
            .catch((error) => {
                ...
                toast.error('Failed to save logo order');
                // Revert to original order on error
                setLogos(indexPage?.logos || []);
            });
    };
    ...
    const handleAddLogo = (e: FormEvent) => {
        ...
        fetch("/admin/index-page/logos", {
            ...
            .then((data: { logo?: Logo }) => {
                ...
                if (data.logo) {
                    setLogos((prev) => [...prev, data.logo as Logo]);
                } else {
                    // Fallback: reload the page if logo data not returned
        ...
        if (editingLogo) {
            ...
            fetch(`/admin/index-page/logos/${editingLogo.id}/update`, {
                ...
                .then((data: { logo?: Logo }) => {
                    ...
                    // Update local state with the updated logo
                    if (data.logo) {
                        setLogos((prev) =>
                            prev.map((logo) =>
                                logo.id === data.logo!.id ? data.logo! : logo
                            )
                        );
                    } else {
                        // Fallback: reload the page if logo data not returned
    ...
    const handleDeleteLogo = (logoId: number) => {
        router.delete(`/admin/index-page/logos/${logoId}`, {
            onSuccess: () => {
                ...
                // Update local state to remove the deleted logo
                ...
                <div className="container mx-auto py-8 px-4">
                    ...
                    <Card className="mt-8">
                        <CardHeader>
                            ...
                            <CardDescription>
                                Manage all logos displayed on the landing page. Drag and drop to reorder.
                            </CardDescription>
                        </CardHeader>
                        <CardContent>
